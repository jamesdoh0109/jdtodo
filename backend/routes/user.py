...
def signup_route():
    ...
    user, message, status_code = create_user_service(signup_json).values()
    if user:
...
def modify_user_route(user_id):
    ...
    user, message, status_code = modify_user_service(modified_user_json, user_id, jwt_user_id).values()
    if user:
        return jsonify({'user': user, 'message': message}), status_code
    else:
        return jsonify({'error': message}), status_code
...
def delete_user_route(user_id):
    jwt_user_id = get_jwt_identity()
    user, message, status_code = delete_user_service(user_id, jwt_user_id).values()
    if user:
...
def change_password_route(user_id):
    ...
    user, message, status_code = change_password_service(change_password_json, user_id, jwt_user_id).values()
    if user:
        return jsonify({'message': message}), status_code
    else:
        return jsonify({'error': message}), status_code
...
def send_reset_password_link_route():
    ...
    user, message, status_code = send_reset_password_link_service(forgot_password_json).values()
    if user:
        return jsonify({'message': message}), status_code
    else:
        return jsonify({'error': message}), status_code
...
def reset_password_route(token):
    ...
    user, message, status_code = reset_password_service(reset_password_json, token).values()
    if user:
        return jsonify({'message': message}), status_code
    else:
        return jsonify({'error': message}), status_code
...
def verify_reset_password_token_route(token):
    user, message, status_code = verify_reset_password_token_service(token).values()
    if user:
        return jsonify({'message': message}), status_code
    else:
        return jsonify({'error': message}), status_code
